...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([getAnalytics(), getTickets({ page_size: 8 })])
      .then(([a, t]) => {
        setAnalytics(a.data)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Tickets"    value={analytics?.total_tickets}    icon={InboxIcon}    color="bg-blue-500" />
        <StatCard label="Open Tickets"     value={analytics?.open_tickets}     icon={AlertCircle}  color="bg-yellow-500" />
        <StatCard label="Resolved"         value={analytics?.resolved_tickets} icon={TicketCheck}  color="bg-green-500" />
        <StatCard label="Avg Resolution"   value={analytics?.avg_resolution_time_hours
          ? `${analytics.avg_resolution_time_hours}h` : 'N/A'}
